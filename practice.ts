...
function addNumbers(n1:number,n2:number):Promise<number>{
  return new Promise((resolve,reject)=>{
    if(typeof n1 === 'number' && typeof n2 === "number"){
      resolve (n1+n2)
    }else{
      reject ('Invalid creandials')
    }
  })
}
...
addNumbers(5,5)
.then((num)=>{
  console.log(`Sum of two numbers is ${num}`)
})
.catch((err)=>{
  console.log(err)
})
...
async function sum(num1: number, num2: number): Promise<number> {
  try {
      const result: number = await addTwoNumbers(num1, num2);
      return result;
  } catch (error) {
...
sum(1, 4).then(console.log);
sum('a' as any, 2).then(console.log);
